(function ($) {
	...
	function require(f) {
		var g = {
			...
			uname: /^[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5_]*$/,
			zh: /^[一-龥]+$/,
			sms: /^[a-zA-Z0-9]{4}$/,
		...
		this.rules = {
			...
			// 验证码不是4位的
			isSms_Sumber: function (a, b) {
				b = b || " ";
				if (!g.sms.test(a)) return b
			},
			// 手机验证码6位
			isInt_phone: function (a, b) {
				b = b || " ";
				if (!g.int_phone.test(a)) return b
			},
...
$(function () {
	...
	$("#myphone").on("input", function () {
		...
		$("#phone_num").text($(this).val());
	});
	$("#adminNo").on("input", function () {

		$("#myadminNo").text($(this).val());
